...
import type { Deduction80GGAModel } from '@/domain/filing/models/deductions/donation/deduction-80gga-model';
import { CLAUSE_TYPES_80GGA, STATES } from '@/domain/utils/master-data';
...
export default function Section80GGATab() {
  ...
  const [entries, setEntries] = useState<Deduction80GGAModel[]>(() =>
    filing.section80Gga ? [...filing.section80Gga] : []
  );
  ...
  const saveEntry = (entry: Deduction80GGAModel) => {
    ...
    if (!entry.clauseUnderDonation) errs.clauseUnderDonation = 'Clause type is required';
    ...
    if ((entry.donationAmountCash || 0) > 10000) errs.donationAmountCash = 'Cash donations above ₹10,000 are not eligible';
    if ((entry.donationAmountCash || 0) === 0 && (entry.donationAmountNonCash || 0) === 0) errs.totalDonationAmount = 'At least one donation amount is required';
  ...
  const clauseOptions = [{ value: '', label: 'Select clause' }, ...CLAUSE_TYPES_80GGA];
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-semibold text-gray-900">80GGA - Scientific Donations</h4>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {isExpanded && (
        <div className="mt-4 space-y-2">
          {entries.length === 0 ? (
            <AddButton label="Add Scientific Donation" onClick={addEntry} colorScheme="purple" />
          ) : (
            entries.map((entry) => {
                  ...
                  {!isEditing ? (
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">PAN: {entry.doneePan} | Total: ₹{formatCurrency(entry.totalDonationAmount || 0)}</p>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <Select label="Clause Under Donation" required value={entry.clauseUnderDonation || ''} onChange={(e) => updateEntry(entry.deductionId, 'clauseUnderDonation', e.target.value)} options={clauseOptions} error={errs.clauseUnderDonation} />
                        ...
                        <Input label="Total Donation Amount" required type="number" value={entry.totalDonationAmount || 0} onChange={(e) => updateEntry(entry.deductionId, 'totalDonationAmount', Number(e.target.value))} prefix="₹" error={errs.totalDonationAmount} />
          ...
          {entries.length > 0 && (
            <button onClick={addEntry} className="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1 mt-1">
              <PlusCircleIcon className="w-4 h-4" /> Add another entry
            </button>
          )}
        </div>
      )}
      <ConfirmModal open={pendingDeleteId != null} title="Delete Entry?" message="Are you sure you want to delete this 80GGA entry?" confirmText="Delete" tone="danger" isLoading={false} onConfirm={() => deleteEntry(pendingDeleteId)} onCancel={() => setPendingDeleteId(null)} />
    </div>
  );
}
