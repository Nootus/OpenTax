...
import type { Deduction80EEAModel } from '@/domain/filing/models/deductions/loan/deduction-80eea-model';
...
export default function Section80EEATab() {
  ...
  const hasEntry = !!filing.section80Eea?.deductionId;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-semibold text-gray-900">80EEA - Affordable Housing</h4>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {isExpanded && (
        <div className="mt-4">
          {!hasEntry && !isEditing ? (
            <AddButton label="Add 80EEA Entry" colorScheme="teal" onClick={() => setIsEditing(true)} />
          ) : (
      ...
      <ConfirmModal open={confirmDeleteOpen} title="Delete Entry?" message="Are you sure you want to delete this Section 80EEA entry?" confirmText="Delete" tone="danger" isLoading={false} onConfirm={handleDelete} onCancel={() => setConfirmDeleteOpen(false)} />
    </div>
  );
}
